import { getCommentsByParentId } from "@/db/comment";
...
export async function getComments(promptId: string) {
    return await unstable_cache(
        async () => {
            const rootComments = await getCommentsByParentId(promptId);
            // Fetch replies for each comment
            const commentsWithReplies = await Promise.all(
                rootComments.map(async (comment) => {
                    const replies = await getCommentsByParentId(comment.id);
